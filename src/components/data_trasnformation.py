...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        try:
            ...
            input_preprocessed_train=preproccessor_obj.fit_transform(input_feature_train_df)
            preprocessed_train_df=pd.DataFrame(input_preprocessed_train,columns=preproccessor_obj.get_feature_names_out())
            input_preprocessed_test=preproccessor_obj.transform(input_feature_test_df)
            preprocessed_test_df=pd.DataFrame(input_preprocessed_test,columns=preproccessor_obj.get_feature_names_out())
            preprocessed_train_df[target_feature]=train_df[target_feature].map({'Fast':2,'Moderate':1,'Slow':0})
            preprocessed_test_df[target_feature]=test_df[target_feature].map({'Fast':2,'Moderate':1,'Slow':0})
            save_object(
            ...
            return (preprocessed_train_df,preprocessed_test_df,self.data_transformation_config.proccessor_obj_file_path)
        except Exception as e:
            raise CustomError(e,sys)